...
export default function AdminDashboard() {
 ...
 return {"Peringkat": index + 1,"Grup Ronda": item.name,"Main (P)": item.played,"Menang (W)": item.won,"Seri (D)": item.drawn,"Kalah (L)": item.lost,"Gol/Poin Memasukkan (GF)": item.goalsFor,"Gol/Poin Kemasukan (GA)": item.goalsAgainst,"Selisih Gol/Poin (GD)": item.goalDifference,"Poin (PTS)": item.points
 };
 } else {
 return {"Peringkat": index + 1,"Grup Ronda": item.name,"Main (P)": item.played,"Menang (W)": item.won,"Seri (D)": item.drawn,"Kalah (L)": item.lost,"Poin (PTS)": item.points
 };
 }
 });
 ...
 // Matches sheet
 const dataMatches = matches.map(item => ({"Hari/Tanggal": item.match_date,"Waktu (WIB)": item.match_time ||"--.--","Grup Kandang (Home)": item.home_group_name,"Skor Kandang": item.home_score !== null ? item.home_score :"-","Skor Tandang": item.away_score !== null ? item.away_score :"-","Grup Tandang (Away)": item.away_group_name,"Babak / Tahap": item.stage ==='group'?'Babak Penyisihan': item.stage,"Status Laga": item.status ==='finished'?'Selesai':'Akan Datang'}));
 ...
 const standingsHeaders = selectedSport?.icon ==='football'? [["POS","GRUP RONDA","MAIN","MENANG","SERI","KALAH","GM","GK","SG","POIN"]]
 ...
 ?"bg-indigo-600 text-white shadow shadow-indigo-500/20":"text-slate-400 hover:text-white hover:bg-slate-800"}`}
 >
 ...
 ?"border-indigo-500 text-indigo-400":"border-transparent text-slate-500 hover:text-slate-200"}`}
 >
 ...
 className="p-2 rounded-xl bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700 transition-all cursor-pointer"title="Edit Match Score / Info">
 ...
 className="p-2 rounded-xl bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500 hover:text-white transition-all cursor-pointer"title="Input Detail Pertandingan">
 {getSportIcon(selectedSport?.icon)}
 </Link>
 )}
 <button
 onClick={() => handleDeleteMatch(m.id)}
 className="p-2 rounded-xl bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-white transition-all cursor-pointer"title="Delete Match">
 ...
 ?"bg-slate-950 border-indigo-500 text-white shadow shadow-indigo-500/10":"bg-slate-900/40 border-slate-800/50 text-slate-400"}`}
 >
 ...
 <input
 type="text"placeholder="Nama pemain baru..."value={newPlayerName}
 ...
 className="p-2 rounded-xl bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500 hover:text-white transition-all cursor-pointer"title="Save changes">
 <Save className="h-3.5 w-3.5"/>
 </button>
 <button
 onClick={() => setEditingPlayerId(null)}
 className="p-2 rounded-xl bg-slate-800 text-slate-400 hover:text-white transition-all cursor-pointer"title="Cancel">
 ...
 className="p-2 rounded-xl bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 transition-all cursor-pointer"title="Edit name">
 <Edit3 className="h-3.5 w-3.5"/>
 </button>
 <button
 onClick={() => handleDeletePlayer(player.id)}
 className="p-2 rounded-xl bg-red-500/10 text-red-450 hover:bg-red-500 hover:text-white transition-all cursor-pointer"title="Delete Player">
 ...
 <input
 type="text"placeholder="19.30"value={matchForm.match_time}
 ...
 <input
 type="number"placeholder="0"min="0"value={matchForm.home_score}
 ...
 <input
 type="number"placeholder="0"min="0"value={matchForm.away_score}
